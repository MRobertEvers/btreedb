...
cleanup_insert(struct SQLParsedInsert* insert)
{
	if( insert->table_name )
		sql_string_free(insert->table_name);

	for( int i = 0; i < insert->ncolumns; i++ )
	{
		if( insert->columns[i] )
			sql_string_free(insert->columns[i]);
	}

	for( int i = 0; i < insert->nvalues; i++ )
	{
		if( insert->values[i].value )
			sql_string_free(insert->values[i].value);
	}
...
{
	if( tbl->table_name )
		sql_string_free(tbl->table_name);

	for( int i = 0; i < tbl->ncolumns; i++ )
	{
		if( tbl->columns[i].name )
			sql_string_free(tbl->columns[i].name);
	}
